...
def progress_hook(data, depth=0):
    ...
    cli.tree_print(
        f"{elapsed}/{rem} {ext} {'◼' * progress}{'◻' * (PROGRESS_LENGTH - progress)} {frs}ETA {eta}", depth, 1
    )
    cli.tree_print(f"- {filepart} -", depth, 2)


def postprocessor_hook(data, depth=0):
    status = data.get("status") or ""
    processes = {"Merger": "Merging", "MoveFiles": "Moving", "FixupM3u8": "Fixing"}
    ...
    if info := data.get("info_dict"):
        ...
        cli.tree_print(f"{dur} {format} ({info.get('id')}) {upload_date}", depth - 1, 1)
        filename = info.get("filename") or ""
        fnsplit = filename.split("/")
        if len(fnsplit) > 1:
            filepart = fnsplit[-1]
    cli.tree_print(f"{processes[postprocess]} {status}", depth, 1)
    cli.tree_print(f"- {filepart} -", depth, 2)


def _make_hooks(depth):
    def _ph(d):
        if d:
            progress_hook(d, depth)
...
def process_download(url_list, availability, depth=0):
    ...
    cli.tree_print(f"{availability}", depth, 1)
    logger = Logger()
    ydl_opts = dict(YDL_OPTS)
    ydl_opts["logger"] = logger
    virt_progress_hook, virt_postprocessor_hook = _make_hooks(depth)
...
def process_channel(ydl, channel, depth=0, index=0):
    ...
    if info:
        ...
        if info_type == "playlist":
            cli.tree_print(f"{info.get('title')}", depth)
            if entries := list(info.get("entries")):
                cli.tree_print(f"{info.get('title')} ({len(entries)})", depth)
                for i, entry in enumerate(entries, start=1):
                    if entry and (entry_url := (entry.get("webpage_url") or entry.get("url"))):
                        process_channel(ydl, entry_url, depth + 1, i)
        elif info_type == "url":
            cli.tree_print(f"URL TODO: {info.get('title')}", depth)
        else:
            video_url = channel
            cli.tree_print(f"|{index:>4}| \033[1m{info.get('title')}\033[0m ({info.get('live_status')})", depth, 0)
            ...
            if not ACCEPT_VERTICAL and util.enumerate_is_vertical(formats):
                cli.tree_print("Vertical", depth + 1, 1)
                ...
                cli.tree_print("Low resolution", depth + 1, 1)
            else:
                availability = info.get("availability")
                process_download([video_url], availability, depth + 1)
            # sleep
            output, duration = util.sleep_calc(int(info.get("duration") or 0))
            if output:
