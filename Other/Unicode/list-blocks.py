...
UNICODE_BLOCKS = [
    ...
    (0x1280, 0x139F, "Ethiopic Supplement"),
...
UNICODE_NONPRINTABLE = [
    *range(0x0000, 0x0020),         # C0 Control
    *range(0x0080, 0x00A0),         # C1 Control
    *range(0x2000, 0x2010),         # Spaces and format
    *range(0x2028, 0x2030),         # Spaces and format
    *range(0x205F, 0x2070),         # Spaces and format
...
UNICODE_DOUBLEWIDTH = [
    ...
    *range(0xFE50, 0xFE70),
    *range(0xFF00, 0xFF60),
    *range(0x17000, 0x187F9),
    *range(0x18800, 0x18CD9),
    *range(0x18800, 0x18CD9),
    *range(0x18D00, 0x18D09),
    *range(0x1AFF0, 0x1B000),
    *range(0x1B000, 0x1B100),
    *range(0x1B100, 0x1B123),
    *range(0x1B170, 0x1B2ED),
    *range(0x1F600, 0x1F650),
    *range(0x20000, 0x323B0),
]

UNICODE_REPLACEMENT = 0xFFFD
UNICODE_LINELENGTH = 32

def check_printable(c):
    if c in UNICODE_NONPRINTABLE:
        return UNICODE_REPLACEMENT
    return c

def add_spacer(c):
    spacer = 1
    if c in UNICODE_MODIFIER:
        spacer += 1
...
def main():
    ...
    for begin, end, description in UNICODE_BLOCKS:
        ...
        if end < 0x10000:
            ...
            header = f"{begin:08X}–{end:08X} {description}"
        print(header + "\n" + "–"*len(header))
        for c in range(begin, end+1):
            if c % UNICODE_LINELENGTH == 0:
                if c > begin:
                    print()
                if end < 0x10000:
                    print(f"{c:04X} | ", end="")
                ...
                if end < 0x10000:
                    print(" "*((begin%UNICODE_LINELENGTH)*3+7), end="")
                else:
                    print(" "*((begin%UNICODE_LINELENGTH)*3+11), end="")
            print(f" {chr(check_printable(c))}" + add_spacer(c), end="")
        print("\n" + "–"*(UNICODE_LINELENGTH*3+11))
